...
import { Star, Calendar, Clock, Film, Tv, Edit2, Trash2 } from 'lucide-react';
...
const ObraDetailsModal = ({ isOpen, onClose, obra, onUpdate, onEdit, onDelete }) => {
  ...
  const handleSaveAvaliacao = async () => {
    ...
    try {
      ...
      if (avaliacao && avaliacao.id) {
        const avaliacaoAtualizada = await avaliacoesService.update(avaliacao.id, payload);
        ...
        <DialogHeader>
          <div className="flex items-center justify-between gap-3">
            <DialogTitle className="text-3xl font-bold text-white flex items-center gap-3">
              {obra.tipo === 'FILME' ? <Film className="w-8 h-8 text-purple-500" /> : <Tv className="w-8 h-8 text-purple-500" />}
              {obra.titulo}
            </DialogTitle>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {obra.generos.map(genero => (
                      <Badge key={genero.id} variant="outline" className="border-purple-500 text-purple-300">
                        {genero.nome}
                      </Badge>
                    ))}
                  </div>
              ...
              {obra.diretores && obra.diretores.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-400 mb-2">Diretor(es)</h4>
                  <div className="flex flex-wrap gap-2">
                    {obra.diretores.map(diretor => (
                      <Badge key={diretor.id} className="bg-gray-800 text-gray-300">
                        {diretor.nome}
                      </Badge>
                    ))}
                  </div>
              ...
              <div className="space-y-4 bg-gray-800/50 p-6 rounded-lg border border-gray-700">
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <Rating
                      value={avaliacao?.nota ? Math.round(parseFloat(avaliacao.nota)) : 0}
                      onValueChange={(value) => setAvaliacao(prev => ({...prev, nota: value.toString()}))}
                      className="gap-0.5"
                    >
                      {[...Array(10)].map((_, i) => (
                        <RatingButton key={i} size={24} className="text-yellow-400" />
                      ))}
                    </Rating>
                    <span className="text-2xl font-bold text-white min-w-[60px]">
                      {avaliacao?.nota ? parseFloat(avaliacao.nota).toFixed(0) : '0'}/10
                    </span>
                  </div>
                  <p className="text-sm text-gray-400 mt-2">Clique nas estrelas para avaliar (1-10)</p>
                </div>

                <div>
                  <Label htmlFor="comentario" className="text-white">Comentário</Label>
                  <Textarea
                    id="comentario"
                    value={comentario}
              ...
              <div className="space-y-3">
                {avaliacao ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-2xl font-bold text-white">{avaliacao.nota}/10</span>
                  </div>
                ) : (
                  <p className="text-gray-400 italic">Nenhuma avaliação ainda</p>
                )}
      ...
      <DialogContent className="max-w-md bg-gray-900 border-red-900/30">
        ...
        <div className="space-y-4 mt-4">
          <p className="text-gray-300">
            Tem certeza que deseja deletar <span className="font-bold text-white">&quot;{obra?.titulo}&quot;</span>?
          </p>
          <p className="text-sm text-gray-400">
            Esta ação não pode ser desfeita. Todas as avaliações associadas também serão removidas.
          </p>
