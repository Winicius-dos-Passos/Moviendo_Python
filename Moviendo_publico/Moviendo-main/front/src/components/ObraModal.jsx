...
import { Film, Tv, Calendar, Clock, Star, Plus, Search } from 'lucide-react';
...
const ObraModal = ({ isOpen, onClose, obra = null, onSuccess }) => {
  ...
  const [isCreatePlataformaModalOpen, setIsCreatePlataformaModalOpen] = useState(false);
  const [isCreateTagModalOpen, setIsCreateTagModalOpen] = useState(false);
  const [isCreateDiretorModalOpen, setIsCreateDiretorModalOpen] = useState(false);
  ...
  const loadOptions = async () => {
    try {
      const [generosData, plataformasData, tagsData, diretoresData] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        anoLancamento: formData.anoLancamento ? parseInt(formData.anoLancamento) : null,
        duracaoMinutos: formData.duracaoMinutos ? parseInt(formData.duracaoMinutos) : null,
        notaImdb: formData.notaImdb ? parseFloat(formData.notaImdb) : null,
        temporadaAtual: formData.temporadaAtual ? parseInt(formData.temporadaAtual) : null,
        episodioAtual: formData.episodioAtual ? parseInt(formData.episodioAtual) : null,
        totalTemporadas: formData.totalTemporadas ? parseInt(formData.totalTemporadas) : null,
        totalEpisodios: formData.totalEpisodios ? parseInt(formData.totalEpisodios) : null,
  ...
  const handleTmdbSelect = async (tmdbData) => {
    ...
    if (tmdbData.generos && tmdbData.generos.length > 0) {
      for (const generoNome of tmdbData.generos) {
        const existingGenero = generos.find(g => g.nome.toLowerCase() === generoNome.toLowerCase());
    ...
    if (tmdbData.diretores && tmdbData.diretores.length > 0) {
      for (const diretorNome of tmdbData.diretores) {
        const existingDiretor = diretores.find(d => d.nome.toLowerCase() === diretorNome.toLowerCase());
        if (existingDiretor && !selectedDiretores.includes(existingDiretor.id)) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="anoLancamento" className="text-white flex items-center gap-2">
            ...
            <div>
              <Label htmlFor="duracaoMinutos" className="text-white flex items-center gap-2">
            ...
            <div className="col-span-2 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {diretores.map((diretor) => (
                  <Badge
                    key={diretor.id}
                    variant={selectedDiretores.includes(diretor.id) ? "default" : "outline"}
                    className={`cursor-pointer transition-all ${
                      selectedDiretores.includes(diretor.id)
                        ? 'bg-purple-600 hover:bg-purple-700 text-white'
                        : 'border-gray-700 hover:border-purple-500 text-gray-300'
                    }`}
                    onClick={() => toggleSelection(diretor.id, selectedDiretores, setSelectedDiretores)}
                  >
                    {diretor.nome}
                  </Badge>
                ))}
            ...
            <div className="col-span-2">
              <Label htmlFor="urlCapa" className="text-white">URL da Capa</Label>
              <Input
                id="urlCapa"
            ...
            <div className="col-span-2">
              <Label htmlFor="sinopse" className="text-white">Sinopse</Label>
              <Textarea
                id="sinopse"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="notaImdb" className="text-white">Nota IMDB (0-10)</Label>
              <Input
                id="notaImdb"
                type="number"
                step="0.1"
                ...
                onChange={(e) => setFormData({ ...formData, notaImdb: e.target.value })}
                className="bg-gray-800 border-gray-700 text-white"
                placeholder="8.7"
              />
            </div>
        ...
        {formData.tipo === 'SERIE' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="temporadaAtual" className="text-white">Temporada Atual</Label>
                <Input
                  id="temporadaAtual"
                  type="number"
                  value={formData.temporadaAtual}
                  onChange={(e) => setFormData({ ...formData, temporadaAtual: e.target.value })}
                  className="bg-gray-800 border-gray-700 text-white"
                  placeholder="1"
                />
              </div>
              ...
              <div>
                <Label htmlFor="episodioAtual" className="text-white">Episódio Atual</Label>
                <Input
                  id="episodioAtual"
                  type="number"
              ...
              <div>
                <Label htmlFor="totalTemporadas" className="text-white">Total de Temporadas</Label>
                <Input
                  id="totalTemporadas"
                  type="number"
                  value={formData.totalTemporadas}
                  onChange={(e) => setFormData({ ...formData, totalTemporadas: e.target.value })}
                  className="bg-gray-800 border-gray-700 text-white"
                  placeholder="4"
                />
              </div>
              ...
              <div>
                <Label htmlFor="totalEpisodios" className="text-white">Total de Episódios</Label>
                <Input
                  id="totalEpisodios"
                  type="number"
                  value={formData.totalEpisodios}
                  onChange={(e) => setFormData({ ...formData, totalEpisodios: e.target.value })}
                  className="bg-gray-800 border-gray-700 text-white"
                  placeholder="34"
                />
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {generos.map((genero) => (
              <Badge
                key={genero.id}
                variant={selectedGeneros.includes(genero.id) ? "default" : "outline"}
                className={`cursor-pointer transition-all ${
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {plataformas.map((plataforma) => (
              <Badge
                key={plataforma.id}
                variant={selectedPlataformas.includes(plataforma.id) ? "default" : "outline"}
                className={`cursor-pointer transition-all ${
                  selectedPlataformas.includes(plataforma.id)
                    ? 'bg-purple-600 hover:bg-purple-700 text-white'
                    : 'border-gray-700 hover:border-purple-500 text-gray-300'
                }`}
                onClick={() => toggleSelection(plataforma.id, selectedPlataformas, setSelectedPlataformas)}
              >
                {plataforma.nome}
              </Badge>
            ))}
